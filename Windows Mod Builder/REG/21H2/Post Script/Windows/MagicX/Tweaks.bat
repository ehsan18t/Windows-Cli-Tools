...
IF NOT EXIST "HWActivation.cmd" (
	COPY /Y "%SystemRoot%\MagicX\Instructions\Finished.txt" "%UserProfile%\Desktop\SUCCESS.txt"
	TASKKILL /IM notepad.exe /F >NUL 2>&1
	RMDIR /S /Q "%SystemRoot%\MagicX\Instructions"
	WScript "%WinDir%\MagicX\Utills\OpenHere.vbs" notepad user "%UserProfile%\Desktop\SUCCESS.txt"
)
